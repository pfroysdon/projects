...
% ts = linspace(0.02,6.5,nc); % theta shift
...
% yc = 0; % circle center in y
% nc = 25; % number of circles
% ts = linspace(0.01,0.5,nc); % theta shift
% rs = r+linspace(0.75,16,nc); % radius shift
...
% yc = 0; % circle center in y
% nc = 25; % number of circles
% ts = linspace(0.02,6,nc); % theta shift
% rs = r+linspace(0.75,16,nc); % radius shift


% cartesian coordinates
% x = (r + a .* sin(n .* theta)) .* cos(theta) + xc; 
% y = (r + a .* sin(n .* theta)) .* sin(theta) + yc;
% plot(x,y,'-b')
...
tool_num = 501;
filename = 'guilloche_style3_tdcZero';
